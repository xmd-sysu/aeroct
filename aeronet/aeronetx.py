...
def parse_v3_site(site, url=None):
    ...
    df = pd.read_csv(io.StringIO(line.decode('utf-8')), skiprows=1)
...
def downlaod_v3_site(site, ymd=None, ymd2=None, prd='AOD15', avg='10',
    ...
    ** Requires curl **
    ...
    prd (str)
        ...
        AOD15    Aerosol Optical Depth Level 1.5 (default)
...
def downlaod_v3_region(llyx, uryx, ymd=None, hr1=0, ymd2=None, hr2=23,
                       prd='AOD15', avg='10', strip_html=True, verb=False):
    """
    ** Note: Requires curl **
    ...
    prd : str, optional
        Product code to download (default 'AOD15'). Accepted prd values are:
        AOD10    Aerosol Optical Depth Level 1.0
...
def parse_v3_web_data(web_data, skip_rows=6):
    ...
    hdr = pd.read_csv(io.StringIO(web_data), skiprows=skipr,
                      nrows=1).columns.tolist()
    hdr[-1] = 'w1'
    hdr.extend(['w' + x for x in map(str, range(2, 13))])

    # read values into data frame
    df = pd.read_csv(io.StringIO(web_data), skiprows=skipr + 1, names=hdr,
...
def plot_v3_site_sda(site, ymd=None, ymd2=None, hr1=0, hr2=23,
    ...
    hdr = pd.read_csv(io.StringIO(data), skiprows=skip_rows,
                      nrows=1).columns.tolist()
    hdr[-1] = 'w1'
    hdr.extend(['w' + x for x in map(str, range(2, 11))])

    # read values into data frame
    df = pd.read_csv(io.StringIO(data), skiprows=skip_rows + 1, names=hdr,
                     na_values=[-999.0], parse_dates={'date_time': [1, 2]},
                     date_parser=date_parse)
    df = df.set_index('date_time')
...
#     show_v3_site()
    ...
    # Example 2: download V3 L15 AOD all points over a specific geo region
    ...
    x = downlaod_v3_region([17.7,72.5],[27.0,90.8], prd='SDA15')
    x_df = parse_v3_web_data(x, skip_rows=5)
    print(x_df.head())
    print(x_df.columns)
